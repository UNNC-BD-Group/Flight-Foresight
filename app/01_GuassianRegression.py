...
Save_Path = '../models/'
Log_Path = '../models/log-2023-04-30-GuassianRegression.log'
...
model_name = 'GuassionRegression'
model = RegressionModel(GeneralizedLinearRegression, trainset, testset)
lr = model.model
grid = ParamGridBuilder().addGrid(lr.family, ['gaussian']).addGrid(lr.tol, [1e-4, 1e-5, 1e-6, 1e-7]).build()
kfolds_rmse = model.kfolds(grid, 3)
